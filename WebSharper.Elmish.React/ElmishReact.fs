...
module Internal =
    ...
    let inline ofType<'T,'P,'S when 'T :> React.Component<'P,'S>> (props: 'P) (children: React.Element seq): React.Element =
        React.CreateElement(WebSharper.React.Helpers.jsConstructor<'T>, props, children |> Array.ofSeq)
...
module ReactNative =
    ...
    module Program =
        ...
        let withReactNative appKey (program:Program<_,_,_,_>) =
            AppRegistry.registerComponent(appKey, fun () -> unbox <| WebSharper.React.Helpers.jsConstructor<App>)
            let setState m d =
                 match appState with
                 | Some state ->
